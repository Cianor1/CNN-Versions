...
Matrix3D create_matrix_3D(int height, int width, int depth) {
    ...
    for (int i = 0; i < height; ++i) {
        ...
        for (int j = 0; j < width; ++j) {
            matrix.data[i][j] = (float*)malloc(depth * sizeof(float));
        }
    }
    return matrix;
}
...
Tensor3D convolve(const Matrix3D* input, const Tensor3D* filters, int stride){
    ...
    int output_height = (padded_input.height - filter_height) / stride + 1; /*(32-3)/2 = 14.5=14*/
    int output_width = (padded_input.width - filter_width) / stride + 1; /*(32-3)/2 = 14.5=14*/
...
Dense create_dense(int input_size, int output_size, float l1_reg){
    ...
    if (layer.input == NULL || layer.weights == NULL || layer.biases == NULL) {
    ...
    for(int i = 0; i < output_size; ++i){
        ...
        if (layer.weights[i] == NULL) {
            fprintf(stderr, "Failed to allocate memory for weight row.\n");
            exit(EXIT_FAILURE);
        }
    }
...
float* dense_forward(Dense* denseLayer, const float* input){
    ...
    for(int i = 0; i < denseLayer->output_size; ++i){
        float sum = denseLayer->biases[i]; /*Current neuron starts with the bias value*/
        #pragma omp parallel for simd reduction(+:sum)
        for(int j = 0; j < denseLayer->input_size; ++j){
            sum += denseLayer->weights[i][j] * input[j]; /*Then update the output by adding the weights times the input*/
        }
        output[i] = sum;
    }
    return output;
...
float L1_Reg_Loss(const Dense* denseLayer){
    ...
    #pragma omp parallel for simd reduction(+:l1_loss)
    for(int i = 0; i < denseLayer->output_size; ++i){
        for(int j = 0; j < denseLayer->input_size; ++j){
...
void train_CNN(Dense* denseLayer, Matrix3D* x_train, float** y_train, int train_size, int batch_size, int epochs, float dropout_rate, Tensor3D* filters) {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        #pragma omp parallel for reduction(+:epoch_loss, epoch_accuracy)
        for (int batch = 0; batch < num_batches; ++batch) {
            float batch_loss = 0.0;
            float batch_accuracy = 0.0;

            #pragma omp parallel for reduction(+:batch_loss, batch_accuracy)
